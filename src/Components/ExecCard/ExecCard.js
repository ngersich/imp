            ...
            <Stack direction="row" sx={{transform: 'translate(-15px, -180px)', /*width: '365px'*/}} >
                <Stack>
                    <Box 
                        ...
                        sx={{cursor: 'pointer'}}
                    />
                    <Typography variant="body1" textAlign="center" fontWeight="bold" width={150} color="primary" display={!display && props.hover ? 'inline' : 'none'}>{props.person.name}</Typography>
                </Stack>
                <Stack sx={{marginLeft: '15px', width:'200px'}} spacing={.5} display={display || !props.hover ? 'flex' : 'none'}>
                    <Typography variant="body1" color="#fff" fontWeight='bold' fontSize="1.5rem">{props.person.name}</Typography>
